...
template <class T> class PathFinders;
...
class Vertex {
    ...
    double dist;           // aux field used in path-finder algs
    Vertex<T>* path;        // aux field used in path-finder algs

	void addEdge(Vertex<T> *dest, double w);
	bool removeEdgeTo(Vertex<T> *d);
	double weightTo(T dest);
public:


	Vertex(T in);
	friend class Graph<T>;
    friend class PathFinders<T>;

	bool operator <(const Vertex<T>* v1) const;
};
...
template<class T>
double Vertex<T>::weightTo(T dest) {
    for (auto& w : this->adj)
        if (w.dest->info == dest)
            return w.weight;
    return -1;
...
template<class T>
bool Vertex<T>::operator<(const Vertex<T> *v1) const {
    return this->dist > v1->dist;
}
